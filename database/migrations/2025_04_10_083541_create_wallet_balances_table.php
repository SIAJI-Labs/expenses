        ...
        if (empty($viewExists)) {
            ...
            DB::statement("
                ...
                SELECT 
                    ...
                    COALESCE(MAX(w.initial_balance), 0)
                        -- Income: Add amount (received by wallet)
                        + COALESCE(SUM(CASE WHEN r.type = 'income' AND r.from_wallet_id = w.id THEN r.amount ELSE 0 END), 0)
                        
                        -- Income - Transfer: Add amount for the receiving wallet (to_wallet)
                        ...
                        -- Expense - Transfer: Subtract (amount + extra_amount) for the sending wallet (from_wallet)
